...
<script type="text/javascript">
	...
	function displayPurchaseResponder() {
	...
	<div class="container container-background">
		...
		<div class="row" id="topCar">
			...
			<?php if (count($items)>0) { ?>
				...
				<?php //$subtotal = str_replace(',', '.', $subtotal); ?>
				...
				<div class="row title-items-car text-right" id="cotizar">
					...
					<?php if ( $user['User']['id_stripe'] == '') { 
						
						$label_button = "Pay with Card"; 

					} else {

						$label_button = "Pay with a new Card"; 

						if ($default_source != '' && $cards != '' ) {
							...
							foreach ($cards as $key => $card) {
								...
								if (  $default_source == $card->id ) {
									...
									//button for purchase with existent card
									echo $this->Html->link('Pay with: ' . $card->brand . '...' . $card->last4, 
										array('controller' => 'Shop', 'action' => 'saveToken'), 
										array('class'=>'btn-add-product-car btn btn-success animated disabled btn-purchase', 'escape'=>false, 'onclick'=>'displayPurchaseResponder()'));
					...
					<!-- CORE10-SW-DEC-4 ... Added ‘User not receiving evaluation feedback on purchase.’ -->
					<?php echo $this->Form->create(null, $options = array('url'=>array('controller'=>'Shop', 'action'=>'saveToken'), 'class'=>'form-initial')); ?>
					  <script
						...
						data-label="<?php echo $label_button; ?>"
						data-description="No charge for 60 days on trials"
